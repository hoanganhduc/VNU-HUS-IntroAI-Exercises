#!/usr/bin/env python3
"""Check only the observable completeness of the Week 3 submission."""
...
def main() -> int:
    ...
    if not is_regular_file(COMPARISON):
        failures.append(
            f"required file {COMPARISON} must be a regular file, not a symbolic link"
        )
    else:
        try:
            text = COMPARISON.read_text(encoding="utf-8")
        except (OSError, UnicodeError) as error:
            failures.append(f"cannot read {COMPARISON} as UTF-8 text: {error}")
        else:
            for name in FIELDS:
                ...
                if value is None:
                    continue
                if not value:
                    failures.append(f"comparison field {name} is empty")
                elif any(token in value for token in PLACEHOLDERS):
                    failures.append(
                        f"comparison field {name} still contains a starter placeholder"
                    )
                else:
                    passes.append(f"comparison field {name} is filled")
